...
import { 
  Bell, Plus, Pencil, Trash2, Calendar,
  Clock, Save, X, BellRing
} from 'lucide-react'
...
function ReminderContent() {
  ...
  const validateForm = () => {
    ...
    if (!formDate) {
      ...
      if (selectedDate < today) newErrors.date = 'Tanggal tidak boleh di masa lalu'
    }
    if (!formTime) newErrors.time = 'Waktu wajib diisi'
  ...
  const ReminderForm = ({ onSave, saveLabel }: { onSave: () => void; saveLabel: string }) => (
    <div>
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="space-y-2">
          <Label htmlFor="form-time">Waktu</Label>
          <Input
            id="form-time"
            type="time"
            value={formTime}
      ...
      <div className="flex gap-2 mt-4">
        <Button onClick={onSave} className="font-mono">
        ...
        <div className="mx-auto max-w-2xl">
          ...
          {isAdding && (
            <Card className="mb-8 border-primary/50">
              ...
              <CardContent>
                <ReminderForm onSave={handleAdd} saveLabel="Simpan" />
              </CardContent>
            </Card>
          )}
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>{reminders.length} reminder tersimpan</CardDescription>
                </div>
                {!isAdding && !editingId && (
            ...
            <CardContent>
              {reminders.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  {!isAdding && (
                    <Button onClick={() => setIsAdding(true)} className="font-mono">
                      <Plus className="mr-2 h-4 w-4" />Buat Reminder Pertama
                    </Button>
                  )}
                </div>
              ) : (
                    ...
                    <div key={reminder.id}>
                      {editingId === reminder.id ? (
                        <div className="p-4 rounded-lg border border-primary/50 bg-primary/5">
                          <ReminderForm onSave={handleUpdate} saveLabel="Update" />
                        </div>
                      ) : (
                        <div className={`flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 rounded-lg border ${
                          ...
                          <div className="flex items-center gap-4">
                            <div className={`flex h-10 w-10 items-center justify-center rounded-full ${
                              isUpcoming(reminder.date, reminder.time) ? 'bg-primary/10' : 'bg-muted'
                            }`}>
                              <Bell className={`h-5 w-5 ${
                                isUpcoming(reminder.date, reminder.time) ? 'text-primary' : 'text-muted-foreground'
                              }`} />
                            </div>
                            <div>
                              <div className="flex items-center gap-2">
                                ...
                                <span className={`font-medium ${
                                  isUpcoming(reminder.date, reminder.time) ? 'text-foreground' : 'text-muted-foreground'
                                }`}>
                              ...
                              <div className="flex items-center gap-2 mt-1">
                                ...
                                <span className="text-sm text-muted-foreground">{formatTime(reminder.time)}</span>
                              </div>
                            </div>
                          </div>
                          <div className="flex items-center gap-2">
                            {!isUpcoming(reminder.date, reminder.time) && (
                              <span className="text-xs px-2 py-1 rounded-full bg-muted text-muted-foreground">Lewat</span>
                            )}
                            <Button variant="ghost" size="icon" onClick={() => handleEdit(reminder)} className="h-8 w-8">
                              <Pencil className="h-4 w-4" />
                            </Button>
                            <Button variant="ghost" size="icon" onClick={() => handleDelete(reminder.id)} className="h-8 w-8 text-destructive hover:text-destructive">
